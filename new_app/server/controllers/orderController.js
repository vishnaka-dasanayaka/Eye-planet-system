...
const getOrder = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    order.frameImg = order.frameImg
      ? `${process.env.API_END_POINT}/Frames/${order.frameImg}`
      : "";
...
const addOrder = asyncHandler(async (req, res) => {
  ...
  try {
    const order = await Order.create({
      ...
      frameImg:
        req.files && req.files.frame_img ? req.files.frame_img[0].filename : "",
    ...
    const prescription = await Prescription.create({
      ...
      presImg:
        req.files && req.files.pres_img ? req.files.pres_img[0].filename : "",
    ...
    const final = await Patient.findByIdAndUpdate(
      req.params.id,
      { $push: { history: `New order added by ${req.user.firstName}` } },
      { new: true }
    );
...
const updateOrder = asyncHandler(async (req, res) => {
  ...
  try {
    const order = await Order.findById(req.params.id);
    const oldPic = order.frameImg;

    const orderDetails = JSON.parse(req.body.orderData);

    if (orderDetails.status === "sent") {
    ...
    const updatedOrder = await Order.findByIdAndUpdate(
      req.params.id,
      {
        lenses: orderDetails.lenses,
        price: orderDetails.price,
        advance: orderDetails.advance,
        balance: orderDetails.price - orderDetails.advance,
        specialNote: orderDetails.specialNote,
        frameDesc: orderDetails.frameDesc,
        status: orderDetails.status,
        sentDate: orderDetails.sentDate,
        deliveredDate: orderDetails.deliveredDate,
        receivedDate: orderDetails.receivedDate,
        frameImg:
          req.files && req.files.frame_img
            ? req.files.frame_img[0].filename
            : "",
      },
      { new: true }
    );
    ...
    if (oldPic !== "") {
      const oldPicPath = path.join(__dirname, "..", "public", "Frames", oldPic);
